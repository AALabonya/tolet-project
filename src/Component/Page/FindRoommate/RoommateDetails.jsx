...
const [center, setCenter] = useState([23.8041, 90.4152]);
    ...
    useEffect(() => {
        setCenter([
            parseFloat(
              roommateDetails?.roomateList?.description?.location?.lat),
            parseFloat(
              roommateDetails?.roomateList?.description?.location?.lon),
        ]);
    }, [roommateDetails]);
        ...
        <div className="mx-auto mt-16 md:px-16 ">
          <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className=" px-5 md:px-0 md:w-3/4">
                <div className="mb-16">
                  ...
                  <div className="relative h-full max-md:min-h-[350px] mt-16">
                    {/* <iframe
                      src={`https://maps.google.com/maps?q=${roommateDetails?.roomateList?.description?.location?.city}&t=&z=13&ie=UTF8&iwloc=&output=embed`}
                      // src={`https://maps.google.com/maps?q=${flatDetails?.flatList?.description?.location?.lat},${flatDetails?.flatList?.description?.location?.lon}&hl=en;z=14&amp;output=embed`}
            ...
            <div className="flex flex-col gap-3 px-6">
              ...
              <div>
               <button onClick={() => setOpenReportModal(true)} className="rounded-md  px-5 py-[6px]  text-black"><div className="md:w-[416px] max-w-[416px] h-fit p-5 underline flex justify-center items-center gap-5">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="1em"
                  height="1em"
            ...
            <div className={`fixed z-[100] flex items-center justify-center ${openReportModal ? 'opacity-1 visible' : 'invisible opacity-0'} inset-0 bg-black/20 backdrop-blur-sm duration-100`}>
          <div className={`absolute max-w-md rounded-lg bg-white p-3 pb-5 text-center drop-shadow-2xl  ${openReportModal ? 'scale-1 opacity-1 duration-300' : 'scale-0 opacity-0 duration-150'} `}>
            <svg onClick={() => setOpenReportModal(false)} className="mx-auto mr-0 w-8 cursor-pointer fill-black " viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g strokeWidth="0"></g><g strokeLinecap="round" strokeLinejoin="round"></g><g><path d="M6.99486 7.00636C6.60433 7.39689 6.60433 8.03005 6.99486 8.42058L10.58 12.0057L6.99486 15.5909C6.60433 15.9814 6.60433 16.6146 6.99486 17.0051C7.38538 17.3956 8.01855 17.3956 8.40907 17.0051L11.9942 13.4199L15.5794 17.0051C15.9699 17.3956 16.6031 17.3956 16.9936 17.0051C17.3841 16.6146 17.3841 15.9814 16.9936 15.5909L13.4084 12.0057L16.9936 8.42059C17.3841 8.03007 17.3841 7.3969 16.9936 7.00638C16.603 6.61585 15.9699 6.61585 15.5794 7.00638L11.9942 10.5915L8.40907 7.00636C8.01855 6.61584 7.38538 6.61584 6.99486 7.00636Z"></path></g></svg>
            <form  >
            ...
            <button onClick={() => addToReportList(roommateDetails)} className="rounded-md bg-indigo-600 hover:bg-indigo-700 px-6 py-1.5 text-white">Ok</button>
          </div>
        </div>
      </div>
            </div>
